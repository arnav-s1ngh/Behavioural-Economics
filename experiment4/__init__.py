...
def creating_session(subsession: Subsession):
    ...
    # Explicitly set buyer and seller roles and track total games
    for group in subsession.get_groups():
        players = group.get_players()
        players[0].is_buyer = True
        players[1].is_buyer = False

        # Increment total games as seller for the seller
...
class Player(BasePlayer):
    ...
    num_false_claims = models.IntegerField(initial=0)
...
def set_payoffs(group: Group):
    ...
    false_claim = False
    if C.tab_key == 0:
        if group.message_table_color1 != 'Red' or group.message_table_color2=='Blue' or group.message_table_color3=='White':
            false_claim = True
    if C.tab_key == 1:
        if group.message_table_color1 == 'Red' or group.message_table_color2!='Blue' or group.message_table_color3=='White':
            false_claim = True
    if C.tab_key == 2:
        if group.message_table_color1 == 'Red' or group.message_table_color2=='Blue' or group.message_table_color3!='White':
            false_claim = True
    # Increment false claims if applicable
    if false_claim:
        seller.num_false_claims += 1
